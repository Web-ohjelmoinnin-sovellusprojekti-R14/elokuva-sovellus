...
async function saveReviewController(rating, user_id, movie_id, comment, media_type) {
  ...
  const review = await pool.query(
    'SELECT * FROM Review WHERE user_id=$1 AND movie_id=$2 AND media_type=$3',
    [user_id, movie_id, media_type]
  );

  if (review.rows.length > 0) {
    await pool.query(
      'UPDATE Review SET rating=$1, comment=$2 WHERE user_id=$3 AND movie_id=$4 AND media_type=$5',
      [rating, comment, user_id, movie_id, media_type]
    );
